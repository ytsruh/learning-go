...
func encryptStream(key string, iv []byte) (cipher.Stream, error) {
	block, err := streamCipherBlock(key)
	if err != nil {
		return nil, err
	}
...
func StreamEncrypt(key, plaintext string) (string, error) {
...
// the original writer.
func StreamEncryptWriter(key string, w io.Writer) (*cipher.StreamWriter, error) {
...
func StreamDecrypt(key, cipherHex string) (string, error) {
...
// not encrypted.
func StreamDecryptReader(key string, r io.Reader) (*cipher.StreamReader, error) {
	iv := make([]byte, aes.BlockSize)
	n, err := r.Read(iv)
	if n < len(iv) || err != nil {
...
func streamCipherBlock(key string) (cipher.Block, error) {
	hasher := md5.New()
